...
1. Distribution of response: $log(\mu_{ij}) = \beta_{1} + \beta_{ij2}(Year_{ij}-1) + \beta_{ij3}Treatment_{i} \times (Year_{ij}-1)$ (I used $Year - 1$ to make it easier to interpret coefficient estimates, so $Year - 1 = (0, 1, 2, 3, 4)$)
...
# For ease of interpretation, re-score year so it starts at zero
skin.data$year <- skin.data$year - 1
...
1. $e^{\beta_{1}}$ : The expected number of new cancers in subjects in the placebo group after one year. 
2. $e^{\beta_{2}(Year - 1)}$ : Factor by which the average number of new cancers changes each year in the study for subjects in the placebo group. i.e. each year the average number of new cancers `r change` by a factor of `r abs(results.table$estimate[2])`, or `r 100 * abs(results.table$estimate[2])`% in subjects in the placebo group. **TODO** CHECK THIS.
