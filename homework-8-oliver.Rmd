...
# TODO: FIX THIS MODEL CONSIDERING CATEGORICAL VARIABLES (Treatment, Center, Skin, Gender)

1. Distribution of response: $log(\mu_{ij}) = \beta_{1} + \beta_{ij2}Year_{ij} + \beta_{ij3}Treatment_{i} + \beta_{ij4}Center_{i} + \beta_{ij5}Age_{i} + \beta_{ij6}Skin_{i} + \beta_{ij7}Gender_{i} + \beta_{ij8}Exposure_{i}$, where
    + Year = {1, 2, 3, 4, 5}
    + Treatment = 0 if placebo, 1 if beta-carotene
    + ## TODO: define remaining covariates
2. Variance of response: $Var(Y_{ij}|X_{ij}) = \phi\mu_{ij}$
3. Correlation structure: $Corr(Y_{ij}, Y_{ik}) = \alpha_{jk}$ (compound symmetric, or exchangable; autoregressive might also be appropriate, though)
...
I found no difference in parameter estimates _or_ standard errors (and therefore Z and p scores) between the original model, accounting for overdispersion (estimated $\phi = `r round(risk.poisson.gee$scale, 3)`$), and the model where I ignored overdispersion (fixed $\phi = 1$).

# TODO: Add table of estimates et al.
