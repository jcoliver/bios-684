...
Using `formula = status ~ time + time*X` excludes the time99:timeX1 term, but brings in the main effect of X into the model. By default, R uses reference level coding (and this is true apparently for `gee`, which has `contr.treatment` for both `time` and `X` in the `contrasts` element of the `gee` output), which is what the `PARAM = REF` command in the `CLASS` Statement of `PROC GENMOD` appears to be doing.
...
```{r joint-wald-test, echo = TRUE, eval = TRUE}
arthritis.joint.wald <- wald.test(b = arthritis.gee.coef[, 1],
                                  varb = arthritis.gee$robust.variance,
                                  Terms = c(6:8))

L.matrix <- matrix(data = NA, nrow = 2, ncol = 8)
L.matrix[1, ] <- c(0, 0, 0, 0, 0, 1, -1, 0)
L.matrix[2, ] <- c(0, 0, 0, 0, 0, 1, 0, -1)
arthritis.joint.wald <- wald.test(b = arthritis.gee.coef[, 1],
                                  varb = arthritis.gee$robust.variance,
                                  L = L.matrix)

arthritis.one.wald <- wald.test(b = arthritis.gee.coef[, 1],
                                  varb = arthritis.gee$robust.variance,
                                  Terms = 6) # <- this gives same result as univariate test
...
arthritis.s.gee <- gee(status ~ sex + time + time:X,
                     id = id,
                     data = arthritis.long,
                     family = binomial,
                     corstr = "unstructured")
arthritis.s.gee.coef <- data.frame(coef(summary(arthritis.s.gee)))
arthritis.s.gee.coef$p <- 2*pnorm(abs(arthritis.s.gee.coef[, 5]), lower.tail = FALSE)
```
