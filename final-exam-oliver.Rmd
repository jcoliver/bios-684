...
```{r marginal-model-arthritis-evaluated, include = FALSE}
...
```{r marginal-model-arthritis-echoed, echo = TRUE, eval = FALSE}
# corstr = "independence" >> effectively *no* within-subject correlation
arthritis.gee <- suppressMessages(gee(status ~ time + time:X,
                     id = id,
                     data = arthritis.long,
...
```{r marginal-model-arthritis-results}
...
**Note:** I'm not convinced this was the best model for this test. A test of the three-way interaction among time, treatment, and sex seems like it would be more appropriate to address the investigator's concern, but attempts at modeling this interaction produced "`rank-deficient model matrix`" and "`error: logistic model for probability has fitted value very close to 1`" errors.
...
```{r new-counts-distribution, echo = TRUE, message = FALSE, fig.height = 2.5}
# Generate values based on normal distribution with mean 0 and s.d. = s.d.(bi)
intercepts <- rnorm(n = 1000, mean = 0, sd = sigma.b)
# Now create data frame based on beta estimates
expected <- data.frame(log.counts = skin.glmm.table$Estimate[1] + 
                         skin.glmm.table$Estimate[2] + 
                         intercepts, 
                       new.counts = NA)
...
hist.plot <- suppressMessages(ggplot(data = expected, mapping = aes(x = new.counts)) +
  geom_histogram() +
