...
The linear mixed effects model, testing the effect of terbinafine and itraconazole on nail growth, treating time as a continuous predictor:
$$
...
And $b_{1i}$ and $b_{2i}$ model the random intercept and slope, respectively, of between-subject variation.

Assumptions on between-subject variation:
$$
b_{i} \sim MVN(0, G) \\
G = 
\begin{pmatrix}
...
Assumptions on within-subject error:
$$
...
**Hypothesis:** $H_{0}: \beta_{3} = 0$

```{r toenail-lmm-treatment, echo = TRUE}
toenail.lmm1 <- lme(fixed = length ~ month + month * treat.cat,
                   random = ~1 + month | id,
                   data = toenail.data)
toenail.lmm1.coef <- coef(summary(toenail.lmm1))
```

#TODO: Run the model, getting estimates and p-values
#TODO: Use autoregressive covariance structure?
