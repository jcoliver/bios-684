...
The odds ratio between average subjects in placebo and treatment groups is then e^`r coef.table$Estimate[6]`^ = `r round(exp(coef.table$Estimate[6]), 3)`.
...
toenail.data$month.cat <- factor(toenail.data$month, 
                                 levels = c("1", "2", "3", "6", "12"))
...
The model, testing the effect of terbinafine and itraconazole on nail growth:
$$
E(Y_{ij}) = \beta_1 + \beta_2X_{1i} \times Trt_i + \beta_2X_{2i} \times Trt_i + \beta_2X_{3i} \times Trt_i + \beta_2X_{4i} \times Trt_i + \beta_2X_{5i} \times Trt_i
$$
Where:
$$
\begin{array}{l}
  X_{1i} = 
  \begin{cases}
    1, & \text{if measurement at one month} \\
    0, & \text{otherwise}
  \end{cases} \\
  X_{2i} = 
  \begin{cases}
    1, & \text{if measurement at 2 months} \\
    0, & \text{otherwise}
  \end{cases} \\
  X_{3i} = 
  \begin{cases}
    1, & \text{if measurement at 3 months} \\
    0, & \text{otherwise}
  \end{cases} \\
  X_{4i} = 
  \begin{cases}
    1, & \text{if measurement at 6 months} \\
    0, & \text{otherwise}
  \end{cases} \\
  X_{5i} = 
  \begin{cases}
    1, & \text{if measurement at 12 months} \\
    0, & \text{otherwise}
  \end{cases} \\
  Trt_i = 
  \begin{cases}
