...
1. Link function:
$$
logit(\mu_{ij}) = \beta_1 + \beta_2 T_{i2} + \beta_3 T_{i3} + \beta_4 T_{i4} + \beta_5 T_{i3} \times X_i + \beta_6 T_{i4} \times X_i + \beta_7 T_{i5} \times X_i
$$
Where:
$$
\begin{array}{l}
...
$$
3. Within-subject association has an unstructured log-odds ratio pattern:
...
***
#QUESTION: How to exclude the `time99:timeX1` term?

```{r marginal-model-arthritis, echo = TRUE}
arthritis.gee <- gee(status ~ time + time:X,
                     id = id,
                     data = arthritis.long,
                     family = binomial,
                     corstr = "unstructured")
...
***
#QUESTION: How to do a multivariate Wald test on GEE?
```{r joint-wald-test, echo = TRUE, eval = FALSE}
arthritis.gee.simple <- gee(status ~ time,
                            id = id,
                            data = arthritis.long,
                            family = binomial,
                            corstr = "unstructured")
...
Test $\beta_{5} = \beta_{6} = \beta_{7} = 0$ via $W^2 = (L\hat{\beta})'\{L\widehat{Cov}(\hat{\beta})L'\}^{-1}(L\hat{\beta})$
...
#QUESTION: How to get $\widehat{Cov}(\hat{\beta})$ from R?
...
***
#QUESTION: the two correlation structures appear the same; from above (emphasis added above)
> Another investigator questions whether it is appropriate to use an _**unstructured log odds ratio correlation**_ structure. Now using PROC GENMOD, fit the same model but using an _**unstructured log odds ratio structure**_...

Maybe this should be a _structured_ working association structure? And what would the structure be? AR?
***
...
***
#QUESTION: How to do this? Including sex as factor or analyzing females alone do not work. 
> `Cgee: error: logistic model for probability has fitted value very close to 1. estimates diverging; iteration terminated.`

***
```{r marginal-compare-sexes, echo = TRUE, eval = FALSE}
males.long <- arthritis.long[arthritis.long$sex == "M", ]
females.long <- arthritis.long[arthritis.long$sex == "F", ]

males.gee <- gee(status ~ time + time:X,
                 id = id,
                 data = males.long,
                 family = binomial,
                 corstr = "unstructured")
males.gee.summary <- summary(males.gee)
males.gee.coef <- coef(males.gee.summary)

# Doesnt work. Too small sample size...?
females.gee <- gee(status ~ time + time:X,
                   id = id,
                   data = females.long,
                   family = binomial,
                   corstr = "unstructured")
females.gee.summary <- summary(females.gee)
females.gee.coef <- coef(females.gee.summary)

# Also causes error ("fitted value very close to 1...")
arthritis.s.gee <- gee(status ~ time + time:X + time:X:sex,
                     id = id,
                     data = arthritis.long,
                     family = binomial,
                     corstr = "unstructured")
