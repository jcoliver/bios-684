...
#TODO: Reduce abstract (currently 1200 characters; needs to be 500)

#### Objective
The objective of this work was to test the effects of auranofin on arthritis.

#### Methods
We used self-assessment of arthritis status in subjects receiving auranofin or placebo treatment as a measurement of effectiveness of treatment. Subjects were randomized before treatment, then asked whether or not they would categorize their arthritis status as "good". This was done one week prior to randomization, at randomization, and at 4, 8, and 12 weeks post-randomization. We used marginal mixed models to assess the effect of treatment and Wald tests to assess significance.

#### Results
We found an effect of the auranofin treatment on the proclivity of the average subject to report having a "good" arthritis status. In a joint Wald test, the average subject receiving auranofin was more likely to have "good" arthritis status than a subject receiving a placebo as measured in weeks 4, 8, and 12 ($W^2 = `r round(arthritis.joint.wald$result$chi2["chi2"], 3)`$, $p = `r round(arthritis.joint.wald$result$chi2["P"], 3)`$). In a joint Wald test, there was no evidence for an effect of sex on the likelihood of a subject reporting "good" arthritis status ($W^2 = `r round(arthritis.s.joint.wald$result$chi2["chi2"], 3)`$, $p = `r round(arthritis.s.joint.wald$result$chi2["P"], 3)`$).
...
#TODO: Compare GEE and GLMM model parameter estimates for intercept. Probably different because GLMM model has random intercepts while GEE does not.
