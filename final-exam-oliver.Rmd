...
Design is balanced, same number and timing of samples for all subjects/treatments
What about sex?

#### Are the data complete? Briefly justify your answer. 

Incomplete. Missing some observations

#### Characterize the pattern of change over time in the proportion with a “good” status by sex.

```{r arthitis-by-sex-over-time}
...
status.by.sex.plot <- ggplot(data = by.sex.table, mapping = aes(x = week, y = prop.good, group = treatment, color = treatment)) +
  # geom_line()
  geom_line() +
  facet_grid(. ~ sex) +
