...
Test $\beta_{5} = \beta_{6} = \beta_{7} = 0$:
...
***
#QUESTION: the two correlation structures appear the same; from above (emphasis added):
> Another investigation questions whether it is appropriate to use an _**unstructured log odds ratio correlation**_ structure. Now using PROC GENMOD, fit the same model but using an _**unstructured log odds ratio structure**_...
