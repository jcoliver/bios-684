...
Testing $\beta_{5} = \beta_{6} = \beta_{7} = 0$ via $W^2 = (L\hat{\beta})'\{L\widehat{Cov}(\hat{\beta})L'\}^{-1}(L\hat{\beta})$
...
arthritis.joint.wald <- wald.test(b = arthritis.gee.coef[, 1], # The vector of parameter estimates (betas)
                                  varb = arthritis.gee$robust.variance, # The variance-covariance matrix
                                  Terms = c(6:8)) # One ore more coefficients to test
```
