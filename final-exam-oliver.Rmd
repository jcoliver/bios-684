...
#TODO: E-mail with question about working correlation structure (independence = no within-subject correlation?)
...
skin.glm.table <- data.frame(round(skin.glm.coef, 3))
skin.glm.table$p.text <- as.character(skin.glm.table$Pr...z..)
skin.glm.table$p.text[skin.glm.table$Pr...z.. < 0.001] <- "< 0.001"
```
...
For this question, I chose to first compare two linear mixed models, a random intercept model and a random intercept and slope model, selecting the best model based on a likelihood ratio test.
...
The random intercept and slopes model, testing the effect of terbinafine and itraconazole on nail growth, treating time as a continuous predictor:
$$
E(Y_{ij}) = \beta_{1} + \beta_{2}Month_{ij} + \beta_{3}Month_{ij} \times Trt_{i} + b_{1i} + b_{2i} \times Month_{ij}
$$
where $b_{1i}$ and $b_{2i}$ model the random intercept and slope, respectively, of between-subject variation.
...
\end{array}
$$
The random intercept model, testing the effect of terbinafine and itraconazole on nail growth, treating time as a continuous predictor:
$$
...
The likeklihood ratio statistic is `r round(lrt.stat, 3)`. Comparing this with a $\chi^2$ distribution with 2 degrees of freedom ($\chi^2_{2, 0.05} = `r round(pchisq(q = 0.05, df = 2, lower.tail = FALSE), 3)`$), the likelihood ratio statistic is significant ($p `r lrt.p.text`$). Therefore, including the random slope in the model provides a significantly better fit than the model with random intercept alone. For all subsequent tests, the models will include random intercept and slopes.

To test the investigator's question, recall the linear mixed model with random intercept and slope is:
...
The question focuses only on those subjects in the terbinafine treatment, so the data are restricted to only those subjects who received terbinafine (N = `r terbin.size`).
