...
```{r continuous-time}
cont.time.model <- gls(FEV1 ~ group + hour + group * hour,
                    data = ozone.long,
                    correlation = corAR1(form = ~1|subject),
                    weights = varIdent(form = ~1|hour),
                    method = "ML")
cont.time.summary <- summary(cont.time.model)
coeffs.95CI <- round(confint(object = cont.time.model), 3)
```
...
The estimated rate of change in the mean response for the room air group is $\beta_{3}$ = `r round(cont.time.summary$coefficients[[3]], 3)`. That is, for every hour in the experiment, we expect the FEV1 to decrease by `r abs(round(cont.time.summary$coefficients[[3]], 3))` hundredths of a mL in the control group.

#### b. What is the estimated rate of change in mean response for the ozone group?

The estimated rate of change in mean response for the ozone group is $\beta_{3} + \beta_{4}$ = `r round((cont.time.summary$coefficients[[3]] + cont.time.summary$coefficients[[4]]), 3)`. That is, for every hour in the experiment, we expect the FEV1 to decrease by `r abs(round((cont.time.summary$coefficients[[3]] + cont.time.summary$coefficients[[4]]), 3))` hundredths of a mL in the control group.

#### c. Test the hypothesis that the rates of change in mean response are identical in the two groups. What do you conclude?

The rates of change in the mean response are not identical in the two groups. We reject this hypothesis ($H_{0}: \beta_{4} = 0$) at p = `r round(cont.time.summary$tTable[4, 4], 3)`.
...
The estimated difference in the rate of mean change between the two groups is $\beta_{4}$ = `r round(cont.time.summary$coefficients[[4]], 3)`. The 95% confidence interval for this estimate is [`r coeffs.95CI[4, 1]`, `r coeffs.95CI[4, 2]`].
...
#TODO: run quatratic model
#TODO: Do model comparisons...
